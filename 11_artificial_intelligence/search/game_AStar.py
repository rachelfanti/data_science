...
class AStarPlayer(OfflinePlayer):
    """
    Offline player using the A* algorithm
    """
    def AStar(self, game_env):
        '''
        Starts the computation for the A* Search
        '''
        ...
        while not priority_queue.empty():
            ...
            for action in game_env.get_viable_actions():
                ...
                if (stored_cost is None) or (game_env.path_cost + game_env.get_heuristic() < stored_cost):
                    reached[(game_env.agent_loc, tuple(game_env.consumed_food))] = game_env.path_cost + game_env.get_heuristic()

                    # The string generated for the ID element avoids error when ordering paths with the same cost
                    priority_queue.put(QueueItem(game_env.path_cost + game_env.get_heuristic(), ''.join(e.name for e in game_env.agent_path), game_env.agent_path)) 

                    if game_env.check_game_status() == GameStatus.OVER:
                        solution = Solution(game_env.path_cost + game_env.get_heuristic(), game_env.agent_path)
    ...
    def start_episode(self, world: EaterWorld) -> Iterable[Eater.Actions]:
        ...
        game_env = GameEnvironment((world.size.x, world.size.y), agent_start_loc, food_locations, block_locations, all_actions, heuristic = chosen_heuristic)
